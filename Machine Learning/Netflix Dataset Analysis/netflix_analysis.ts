...
function countNulls(data: any[], column: string): number {
...
function sortMapByValue<K>(map: Map<K, number>, descending: boolean = true): [K, number][] {
    const entries = Array.from(map.entries());
    return entries.sort((a, b) => descending ? b[1] - a[1] : a[1] - b[1]);
}

function printHorizontalBar(label: string, value: number, maxValue: number, barWidth: number = 40): void {
...
function loadDataset(filePath: string): NetflixTitle[] {
    ...
    return parsed.data as NetflixTitle[];
}
...
function analyzeMissingValues(data: NetflixTitle[]): void {
    printSeparator();
...
function cleanDataset(data: NetflixTitle[]): CleanedTitle[] {
    printSeparator();
    ...
    const uniqueMap = new Map<string, CleanedTitle>();
...
function printSummaryStatistics(data: CleanedTitle[]): void {
    printSeparator();
...
function visualizeContentTypeDistribution(data: CleanedTitle[]): void {
    printSeparator();
...
function visualizeTopRatings(data: CleanedTitle[], top: number = 10): void {
    printSeparator();
...
function visualizeTopGenres(data: CleanedTitle[], top: number = 10): void {
    printSeparator();
...
function visualizeTopCountries(data: CleanedTitle[], top: number = 10): void {
    printSeparator();
...
function visualizeReleasesByYear(data: CleanedTitle[], lastNYears: number = 15): void {
    printSeparator();
...
function visualizeYearOverYearGrowth(data: CleanedTitle[], lastNYears: number = 10): void {
    printSeparator();
    ...
    for (const year of recentYears) {
        ...
        if (prevCount > 0) {
            ...
            if (growthRate > 20) trend = '▲▲ Strong growth';
            else if (growthRate > 0) trend = '▲ Growth';
            else if (growthRate > -20) trend = '▼ Decline';
            else trend = '▼▼ Sharp decline';
        } else {
            growth = 'N/A';
            trend = '— Baseline';
        }
...
function printDurationAnalysis(data: CleanedTitle[]): void {
    printSeparator();
...
function printSampleData(data: CleanedTitle[], n: number = 5): void {
    printSeparator();
...
function main(): void {
    ...
    let data: NetflixTitle[] | null = null;
    let usedPath = '';
