...
function oneHotEncode(X: (number | string)[][], columnIndex: number): number[][] {
    // Extract unique categories from the specified column, sorted
    const categories = [...new Set(X.map(row => row[columnIndex] as string))].sort();
...
class MultipleLinearRegression {
    ...
    fit(X: number[][], y: number[]): void {
        ...
        const X_aug: number[][] = X.map(row => [1, ...row]);
...
function main() {
    ...
    const csvFile = fs.readFileSync(
        'C:\\Users\\SriniAchuthan\\OneDrive\\Desktop\\Machine-Learning-A-Z-Codes-Datasets\\Machine Learning A-Z\\Part 2 - Regression\\Section 5 - Multiple Linear Regression\\Python\\50_Startups.csv',
        'utf8'
    );
    ...
    const X_raw: (number | string)[][] = dataset.map(row =>
    ...
    const X_encoded: number[][] = oneHotEncode(X_raw, 3);
